...
public class LocalDummyDataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        Operator op1 = operatorRepository.save(
                Operator.of("operator1", "{noop}local", OperatorRole.OPERATOR));
        Operator op2 = operatorRepository.save(
                Operator.of("operator2", "{noop}local", OperatorRole.ADMIN));
    ...
    private void seedInquiry(InquiryStatus status, String userId, String content,
        ...
        Inquiry inquiry = Inquiry.of(
                Map.<String, Object>of("userId", userId, "nickname", userId + "님", "channel", "WEB"),
        ...
        if (urgency != null) {
            ...
            diagnosisRepository.save(Diagnosis.of(inquiryId, cause, direction, 0.82));
        }

        // 초안 (검토중/승인/발송 단계)
        if (withDraft) {
            draftResponseRepository.save(DraftResponse.of(inquiryId,
